...
%     if contains(gpuDevice(1).Name,'GeForce')
% %         z = gpuArray(single(z));
% %         alpha = gpuArray(single(alpha));
% %         x = gpuArray(single(x));
% %         ddx = gpuArray(single(ddx));
% %         sumI = gpuArray(single(sumI));
% %         sumP = gpuArray(single(sumP));
% %         temp_j = gpuArray(single(temp_j));
% %         temp_k = gpuArray(single(temp_k));
%     else
        z = gpuArray(z);
...
% MI = log2(M) - 1/M/pi*sum(sumI)/log(2);
Msum = numel(pos_idx);
